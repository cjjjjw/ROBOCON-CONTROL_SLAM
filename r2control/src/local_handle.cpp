 ...
 * return: data_vel的指针
 */
unsigned char *R2serHandler::ASCIITranform(double data,int len, int num_postion)
{
    unsigned char *data_vel=(unsigned char*)malloc(len);//增加一个数据位
    int pos_xc;
    int nums_length=len-1 ; //后退到几位
    //直接用ascii码代替数字
    //正反判断
    if (data<0){
        data_vel[0]=0x2d;
    ...
    pos_xc=(int)(round(data*pow(10,nums_length-num_postion)));//数值长度为nums_length，当数值为1时，10000；改为-2，数值为1时，01000，往后推一位
    pos_xc=abs(pos_xc);
    //数值转ascii的处理
...
unsigned char *R2serHandler::writeSpeedNow(double pos_x_now,double pos_y_now,double omega)
{
    ...
    static unsigned char buf_NOW[24] = {};
    // unsigned char *buf_NOW=(unsigned char*)malloc(22);
    //初始化数组
    memset(buf_NOW,0x00,24);
    int length;
    //五位数据，第一位是符号位
    unsigned char *postion_data_now_x =R2serHandler::ASCIITranform(pos_x_now,6,2);
    unsigned char *postion_data_now_y=R2serHandler::ASCIITranform(pos_y_now,6,2);
    // unsigned char *postion_data_now_yaw=R2serHandler::ASCIITranform(yaw_now,3);
    unsigned char *omeag_now=R2serHandler::ASCIITranform(omega,6,1);

    for(int i = 0; i < 2; i++)
        buf_NOW[i] = this->header->header_speed_now[i];             //buf[0]  buf[1]
    // 设置机器人前进旋转
    
    for(int i = 0; i < 6; i++)
    {
        buf_NOW[i + 2] = *(postion_data_now_x+i);    //buf[2] buf[3] buf[4] buf[5] buf[6] buf[7] x_postion
        buf_NOW[i + 8] = *(postion_data_now_y+i);  //buf[8] buf[9] buf[10] buf[11] buf[12] buf[13] y_postion
        buf_NOW[i + 14] = *(omeag_now+i);//buf[14] buf[15] buf[16] buf[17] buf[18] buf[19] omega
    ...
    buf_NOW[20] = this->header->ctrlFlag;       //buf[20]
    
    // 设置校验值、消息尾
    length=21;  //length=buf[21]

    buf_NOW[length] = getCrc8(buf_NOW, length);//buf[22]
    buf_NOW[length + 1] = this->header->ender[0];     //buf[23]
    buf_NOW[length + 2] = this->header->ender[1];     //buf[24]
...
unsigned char *R2serHandler::writeSpeedGoal(double pos_x_goal,double pos_y_goal,double omega)
{
    ...
    // 设置消息头
    for(int i = 0; i < 2; i++)
        buf_Goal[i] =this->header->header_speed_goal[i];             //buf[0]  buf[1]
    
    // 设置机器人前进旋转
    //五位数据，第一位是符号位
    postion_data_goal_x=R2serHandler::ASCIITranform(pos_x_goal,5,1);
    postion_data_goal_y=R2serHandler::ASCIITranform(pos_y_goal,5,1);
    // postion_data_goal_yaw=R2serHandler::ASCIITranform(yaw_goal,3);
    omega_goal=R2serHandler::ASCIITranform(omega,5,1);
...
unsigned char *R2serHandler::writePosNow(double pos_x_goal,double pos_y_goal,double yaw)
{
    ...
    int length;
    //五位数据，第一位是符号位
    unsigned char *postion_data_now_x =R2serHandler::ASCIITranform(pos_x_goal,6,2);
    unsigned char *postion_data_now_y=R2serHandler::ASCIITranform(pos_y_goal,6,2);
    // unsigned char *postion_data_now_yaw=R2serHandler::ASCIITranform(yaw_now,3);
    unsigned char *omeag_now=R2serHandler::ASCIITranform(yaw,6,1);

    for(int i = 0; i < 2; i++)
        buf_NOW[i] = this->header->header_pos_now[i];             //buf[0]  buf[1]
    // 设置机器人前进旋转
...
unsigned char *R2serHandler::writePosGoal(double pos_x_goal,double pos_y_goal,double yaw)
{
    ...
    // 设置消息头
    for(int i = 0; i < 2; i++)
        buf_Goal[i] =this->header->header_pos_goal[i];             //buf[0]  buf[1]
    ...
    postion_data_goal_y=R2serHandler::ASCIITranform(pos_y_goal,5,1);
    // postion_data_goal_yaw=R2serHandler::ASCIITranform(yaw_goal,3);
    omega_goal=R2serHandler::ASCIITranform(yaw,5,1);
